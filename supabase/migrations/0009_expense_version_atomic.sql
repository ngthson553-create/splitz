-- Splitz — Slice 4 (QC fix): chống ghi đè mù THỰC SỰ atomic.
-- 0008 kiểm version bằng SELECT-trước-rồi-UPDATE-sau (WHERE chỉ có id) → ở READ COMMITTED
-- 2 người sửa cùng version có thể cùng qua pre-check, người sau ghi đè người trước mà
-- KHÔNG raise CONFLICT (đúng lỗi giết Hysplit cũ).
-- Sửa: đưa version vào WHERE của UPDATE (khoá row, kiểm + ghi nguyên tử); 0 row → CONFLICT.
...
begin
  ...
  if p_expense_id is null then
    ...
    -- Tồn tại + quyền (chủ nhóm sửa mọi khoản; thành viên chỉ sửa khoản do mình tạo).
    ...
    -- Kiểm version NGUYÊN TỬ ngay trong UPDATE: row bị khoá, version phải khớp.
    update public.expenses set
      ...
      where id = p_expense_id
        and (p_expected_version is null or version = p_expected_version);  -- trigger touch_row tăng version
    if not found then
      -- Version đã đổi (có người vừa sửa) → không ghi đè mù.
      raise exception 'CONFLICT: khoản chi vừa được cập nhật.' using errcode = 'P0001';
    end if;
    v_id := p_expense_id;

    -- Thay toàn bộ dòng con (item_participants cascade theo item).
